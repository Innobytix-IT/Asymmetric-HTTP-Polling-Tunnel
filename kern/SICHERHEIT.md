...
**Am 03.09.2026 entfernt:** Der Selbsttest gab frueher `geheim_pruef` aus --
die ersten 16 Hexzeichen von `sha256(Geheimnis)`, gedacht als Hilfe beim
Einrichten. Der Wert war **ohne jede Anmeldung abrufbar** und damit ein
Offline-Rateorakel: Kandidaten liessen sich milliardenfach je Sekunde
dagegen pruefen, ohne den Server anzufassen.

Bei einem zufaelligen 64-Zeichen-Geheimnis ist das folgenlos. Geprueft wird
aber nur die **Laenge** (mindestens 16 Zeichen), nicht die Zufaelligkeit --
`sommer2026abcdef` kommt durch, und fuer so eines waere der Wert das Ende.

Ob zwei Seiten dasselbe Geheimnis haben, prueft `ausliefern.sh` jetzt,
indem es sich damit **ausweist**: 403 heisst "passt nicht", alles andere
"passt". Ein Test ist besser als ein Fingerabdruck.
...
*Nachgemessen 03.09.2026:* Gegen echtes Apache tritt die Falle auf keinem
der beiden verwendeten Hoster auf -- ETags dort tragen die Aenderungszeit
mit Unterauflaesung (bplaced: `1e-65a873fcf129c` gegen `1e-65a873fdb3802`
bei gleicher Groesse; IONOS: `1f-65a87400c35a9`). Der Befund stammte aus
der Attrappe, die Apaches dokumentierte Vorgabe pessimistisch nachbildet.
Die Schranken bleiben als Vorsorge fuer Hoster mit sekundengenauer
Aenderungszeit.

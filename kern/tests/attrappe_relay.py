...
class Ablage:
    ...
    def _raeume(self, marke, stuecke):
    ...
    def _kehre(self):
        ...
        for e in self.offen:
            if jetzt - e['ts'] > MARKE_TTL:
                self._raeume(e['marke'], e.get('st', 0))
            else:
                behalten.append(e)
        self.offen = behalten
        behalten = []
    ...
    def _warteschlange(self):
        ...
        self._schreibe('warteschlange.json', json.dumps(
            {'stand': int(time.time()), 'folge': self.folge,
             'offen': [{'marke': e['marke'], 'ts': e['ts']} for e in self.offen]},
            ensure_ascii=False))

    def _umschlag(self, marke, teil, teile, nutzlast):
        return json.dumps({'v': VERSION, 'marke': marke, 'teil': teil,
                           'teile': teile, 'krypto': 'keine',
    ...
    def antwort(self, eingabe):
        ...
        with self.sperre:
            ...
            self._schreibe('antwort_%s.json' % marke,
                           self._umschlag(marke, 0, teile, nutz))
            try:
...
class Behandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if aktion != 'selbsttest':
            ...
            if eingabe.get('krypto', 'keine') != 'keine':

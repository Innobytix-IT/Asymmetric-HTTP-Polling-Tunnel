...
### Nachgemessen am 03.09.2026 -- und der Befund faellt milder aus

Die Kollision oben wurde gegen `tests/attrappe_relay.py` gezeigt, und die
bildet Apaches DOKUMENTIERTE Vorgabe nach: `int(st.st_mtime)`, also ganze
Sekunden. Echtes Apache 2.4 nimmt die Aenderungszeit aber mit
Unterauflaesung in den ETag, wenn das Dateisystem sie liefert. Gemessen an
beiden verwendeten Hostern:

```
bplaced   zwei gleich grosse Schreibvorgaenge, dieselbe Sekunde:
            1e-65a873fcf129c   und   1e-65a873fdb3802   -> VERSCHIEDEN
IONOS     laufende Warteschlange:  1f-65a87400c35a9
```

Gleiche Groesse (`1e` = 30 Bytes), verschiedene Zeit. **Auf diesen beiden
Hostern tritt die Kollision nicht auf**, auch nicht in der laufenden
OWLP-Installation.

Die frueher hier stehende Behauptung, dieselbe Luecke stecke in
`tunnel.php`, war damit zu weit gegriffen: Sie ist moeglich, aber auf
keinem der gemessenen Hoster wirksam.

**Die drei Schranken bleiben trotzdem**, und zwar als Vorsorge statt als
Reparatur: Es gibt Dateisysteme und Konfigurationen mit sekundengenauer
Aenderungszeit, und der unbedingte Abruf kostet eine kleine statische Datei
alle 30 Sekunden. Was nichts kostet und einen ganzen Fehlerfall abdeckt,
baut man nicht wieder aus -- aber man behauptet auch nicht, es behebe etwas
Belegtes.

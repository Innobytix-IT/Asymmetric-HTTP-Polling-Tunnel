# AHPT Cloud 3000 — Architektur

Stand 02.09.2026; Abschnitt 2a zur Transport-Schicht ergaenzt am
07.09.2026. Dieses Dokument entscheidet, wie aus der OWLP-Sonderloesung
...
### 2a. Die Schicht, die AHPT Cloud 3000 einzieht

Zwischen Agent und Vermittler steht hier eine weitere Schicht:
`transport.py`. Der Agent sagt nur noch *was* abzulegen ist, nicht mehr
*wie*.

```
[ Agent zu Hause ]     relay_agent.py  --  kennt keinen Dienst UND keinen Weg
     |  lege_frage / hole_offene_marken / lege_antwort / ...
     v
[ Transportwege ]      transport.py    --  PhpTransport | WebDavTransport
     |                                     + Gesundheit (Kreislauf-Unterbrecher)
     v
[ Vermittler ]         relay.php  ODER  ein WebDAV-Speicher ohne eigenen Code
```

**Warum ueberhaupt eine Abstraktion, wo doch eine genuegte?** Nicht der
Vielfalt wegen. Der Grund ist, dass das Kostenmodell oben eine
*Eigenschaft von PHP-Webspace* ist, keine des Verfahrens. Ein
WebDAV-Speicher hat ein voellig anderes: Dort ist ein Schreibvorgang
nicht teuer, dafuer gibt es gar keinen Prozess, der etwas fuer uns tun
koennte. Damit kehrt sich die Arbeitsteilung um:

| | PHP-Weg | WebDAV-Weg |
|---|---|---|
| Buchfuehrung, Deckel, Verfall | `relay.php` | **niemand** — entfaellt |
| Warteschlange | Datei, vom Vermittler geschrieben | `PROPFIND` auf das Verzeichnis |
| Grosse Nutzlast | gestueckelt, ~48 KB je Stueck | **ein** PUT, gemessen bis 12 MB |
| Aufraeumen | Vermittler, nach Verfall | Agent und Besucher selbst |
| Ausweis | `X-AHPT-Auth` | Konto und Passwort (Basic) |

Alles, was in der linken Spalte serverseitig geschieht, muss rechts im
Transportweg selbst stecken. Das ist **keine bessere Bauart** — es ist
die Anpassung an einen Speicher, der keine andere Wahl laesst. Der
Gewinn steht in der dritten Zeile: keine Stueckelung.

**Warum mehrere Wege gleichzeitig, statt einen zu waehlen?** Weil ein
einzelner Vermittler ein einzelner Ausfall *und* ein einzelner Mitwisser
ist, und weil ein WebDAV-Konto zusaetzlich jederzeit gesperrt werden
kann (`SICHERHEIT.md` 3.7). Jede Nachricht sucht sich der Reihe nach
einen Weg, der gerade traegt.

Dazu gehoert `transport.Gesundheit`: Ein dauerhaft toter Weg darf nicht
bei **jeder** Nachricht erneut seine volle Frist verstreichen lassen —
sonst kostet die Redundanz mehr, als sie einbringt, und zwar fuer immer,
bis ein Mensch es bemerkt. Nach zwei Fehlschlaegen wird ein Weg
voruebergehend gesperrt, die Sperre waechst mit jedem weiteren
Fehlschlag und ist gedeckelt; danach genau **ein** Testversuch. Ein
Erfolg setzt die Zaehlung zurueck, nicht nur die Sperre — sonst spraenge
der naechste einzelne Aussetzer sofort auf den gedeckelten Wert.

**Was die Schicht ausdruecklich nicht tut:** Sie kennt keinen Dienst,
keine Fassung des Protokolls und keine Verschluesselung. Die
Zerlegung des Geheimtexts in Stuecke geschieht *ausserhalb*, am fertigen
Geheimtext; ob eine zu grosse Antwort durch eine Fehlermeldung ersetzt
werden darf, entscheidet der Agent, nicht der Weg. Ein Transportweg, der
in die Nutzlast sieht, waere derselbe Fehler wie ein `relay.php`, das
Fragen beantwortet.
